...
\newcommand{\E}{\mathcal{E}} % error map
\newcommand{\PauliSet}{\mathcal{P}} % set of Pauli operators
\newcommand{\U}{\mathcal{U}} % ideal operation
...
of qubit indices, let $\PauliSet^S$ denote the set of *unphased* Pauli operators acting on those
qubits. Note that we think of elements of $\PauliSet^S$ as functions mapping $S \rightarrow \{I, X,
Y, Z\}$, so that for any $P \in \PauliSet^S$ and $T \subseteq S$, $P|_T$ denotes the restriction of
$P$ to the qubit subset $T$. Along these lines, for disjoint sets $S, T$, and $P \in \PauliSet^S$
and $Q \in \PauliSet^T$, $P \otimes Q$ denotes the element of $\PauliSet^{S \cup T}$ such that $P
\otimes Q |_S = P$ and $P \otimes Q |_T = Q$. This notation is helpful to avoid explicitly dealing
with subsystem orderings, and to make it easy to describe restrictions.
...
$$ \E_m = \frac{1}{2^{2|M| + |N|}}\sum_{x, y \in \Z_2^M, Q \in \PauliSet^N} (-1)^{m \cdot (x + y)}
...
\PauliSet^{N\setminus R}}} (-1)^{m \cdot (x + y|_M)} \lambda^Q_{x,y}  \opket{Q \otimes
...
For any $x \in \Z_2^M$, $y \in \Z_2^{M \cup R}$, and $Q\in \PauliSet^{N \setminus R}$, it holds that:
...
\Z_2^M, y \in \Z_2^{M \cup R} \\ Q \in \PauliSet^{N\setminus R}}} \lambda^Q_{x,y}  \opket{Q \otimes Z^y}
...
&= \opbra{Q \otimes Z^y} \otimes \opbra{Z^{x + y|_M}} \left( \frac{1}{2^{2|M|+|N|}} \sum_{\substack{a \in \Z_2^M, b \in \Z_2^{M \cup R} \\ P \in \PauliSet^{N\setminus R}}} \lambda^P_{a,b}  \opket{P \otimes Z^b}\opbra{G^\dagger(P \otimes I_{N \cap R} \otimes Z^a)} \otimes \opket{Z^{a + b|_M}} \right) \\
&=\frac{1}{2^{2|M| + |N|}}\sum_{\substack{a \in \Z_2^M, b \in \Z_2^{M \cup R} \\ P \in \PauliSet^{N \setminus R}}} \lambda^P_{a, b}\underbrace{\ip{Q \otimes Z^y}{P \otimes Z^b}}_{2^{|M| + |N|} \delta_{P, Q}\delta_{y,b}} \underbrace{\ip{Z^{x + y|_M}}{Z^{a + b|_M}}}_{2^{|M|}\delta_{x+y|_M,a+b|_M}}\opbra{G^\dagger(P \otimes I_{N \cap R} \otimes Z^a)}
\end{aligned}
$$
