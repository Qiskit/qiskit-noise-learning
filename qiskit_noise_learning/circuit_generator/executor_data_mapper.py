...
class ExecutorDataMapper:
    ...
    Args:
        item_sequence_indices: For each program item, an ordered list of instruction sequence
            ...
            item.
        item_registers: For each program item, its classical registers, ordered so that
    ...
    def item_registers(self) -> list[list[MeasurementRegister]]:
        """Per program item, the classical registers contained in the results.

        The registers at a given index describe the data of the
        :class:`qiskit_ibm_runtime.results.QuantumProgramResult` at the same index, and are ordered
        so that concatenating their outcomes gives the ``"bit"`` dimension of a
        :class:`~.RawData`.
        """
