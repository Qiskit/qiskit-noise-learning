...
class PathPattern(BasePattern[FidelityIndex]):
    """A pattern for a sequence of fidelity indices."""
    ...
    def extend_permutations(
        self, instruction_pattern: InstructionPattern
    ) -> InstructionPattern | None:
        r"""Return an instruction pattern with extended permutations to traverse self.

        Given an ``instruction_pattern``, this method attempts to "extend" its definition by
        construct a new instruction pattern with the same structure as the input, but for which the
        :class:`PartialPauliPermutation`\s are extended to specify the minimal superset of mappings
        required to traverse this path. If this is not possible, the function will return ``None``,
        indicating that the procedure failed.

        In terms of the specific algorithm, when encountering a :class:`PartialPauliPermutation`
        while jointly iterating through ``self`` and ``instruction_pattern``, this method will
        ...
        instruction pattern that traverses the path.

        Args:
            instruction_pattern: The base instruction pattern.

        Returns:
            The extended instruction pattern, or ``None`` if an extension is not possible.
        ...
        new_start_fragment = _extend_instruction_fragment(
            self.start_fragment,
            instruction_pattern.start_fragment,
        ...
        new_repeatable_fragment = _extend_instruction_fragment(
            ...
            instruction_pattern.repeatable_fragment,
        ...
        new_end_fragment = _extend_instruction_fragment(
            self.end_fragment,
            instruction_pattern.end_fragment,
            input_pauli=expected_start_output,
    ...
    def is_traversed_by(self, instruction_pattern: InstructionPattern) -> bool:
        """Whether or not this path is traversed by the instruction pattern.
        ...
        ``instruction_pattern``, they must have similar structure, in the sense that every
        :class:`FidelityIndex` in a fragment of ``self`` has a corresponding :class:`ApplyGate`
        instruction in the corresponding fragment of ``instruction_pattern``. This precludes this
        ...
        :class:`InstructionPattern` with all instructions in the ``end_fragment``, even if the
        experiment specified by the :class:`InstructionPattern` does in fact traverse the path. This
        limitation is purposefully chosen to simplify the logic of the function, under the
        assumption that these circumstances are unlikely to arise in any algorithm written to
        construct paths and instruction patterns.

        Args:
            instruction_pattern: The instruction pattern.
        ...
        if instruction_pattern.start_fragment:
        ...
        if instruction_pattern.repeatable_fragment:
        ...
        if instruction_pattern.end_fragment:
            current_pauli = _instruction_fragment_traverses_path_fragment(
                current_pauli,
                self.end_fragment,
                instruction_pattern.end_fragment,
    ...
    def sign_flips(self, instruction_pattern: InstructionPattern) -> tuple[bool, bool]:
        """Whether the instruction pattern fragments flip the observable sign when traversing self.

        Requires the path starts and ends at the identity.

        Args:
            instruction_pattern: An instruction pattern that traverses this path.
        ...
        Raises:
            ValueError: If the path doesn't satisfy the assumptions, or the instruction pattern does
                not actually traverse the path.
        """
        ...
        start_error = ValueError(
            "Cannot compute signs as instruction pattern start fragment does not traverse "
            "path pattern start fragment."
        )
        if instruction_pattern.start_fragment:
            out_pauli = _instruction_fragment_traverses_path_fragment(
                current_pauli,
        ...
        repeatable_error = ValueError(
            "Cannot compute signs as instruction pattern repeatable fragment does not traverse "
            "path pattern repeatable fragment."
        )
        # default value
        repeatable_flip = False
        if instruction_pattern.repeatable_fragment:
        ...
        end_error = ValueError(
            "Cannot compute signs as instruction pattern end fragment does not traverse "
            "path pattern end fragment."
        )
        if instruction_pattern.end_fragment:
            out_pauli = _instruction_fragment_traverses_path_fragment(
                current_pauli,
    ...
    def to_instruction_pattern(self) -> InstructionPattern:
        r"""Return an instruction pattern that traverses this path.

        The returned pattern is minimally specified, in the sense that the layers of single qubit
        ...
        The algorithm to construct the output :class:`.InstructionPattern` proceeds as follows. Let
        ...
        * A final :class:`.PartialPauliPermutation` is appended to ``repeatable_fragment`` to
          ...
          ``self.repeatable_fragment[0]``. This ensures the returned instruction pattern can
        ...
        * No additional boundary modifications are required.
        ...
        Returns:
            An instruction pattern traversing this path pattern.
        """
    ...
    def _validate_starts_and_ends_with_identity(self) -> QubitSparsePauli:
        ...
        if start_pauli != QubitSparsePauli.identity(start_pauli.num_qubits):
            raise ValueError("Path pattern does not begin with identity.")
        ...
        if final_pauli != QubitSparsePauli.identity(final_pauli.num_qubits):
            raise ValueError("Path pattern does not end with identity.")
...
def _path_fragment_to_instruction_fragment(path_fragment: list[FidelityIndex]) -> list[Instruction]:
    """Given a path fragment, build a minimal instruction fragment that traversing it.

    See the documentation for :meth:`PathPattern.to_instruction_pattern` for a description of the
    overall algorithm.
    """
