...
def test_exponential_fit_curves_extracts_base_and_spam_intercept(
    make_cz_path, make_aggregated_observable_data
):
    p = make_cz_path("XI")
    averaged = make_aggregated_observable_data([(p, -1, 0.8, {"spam_fidelity": 0.95})])
    bases, intercepts = exponential_fit_curves(averaged)
    assert bases[p] == pytest.approx(0.8)
    assert intercepts[p] == pytest.approx(0.95)


def test_exponential_fit_curves_intercept_defaults_to_one(
    make_cz_path, make_aggregated_observable_data
):
    p = make_cz_path("XI")
...
def test_exponential_fit_curves_ignores_point_rows(make_cz_path, make_aggregated_observable_data):
    p = make_cz_path("XI")
    # A fragment_depth>=0 point row is not an exponential-fit (fragment_depth == -1) entry.
    averaged = make_aggregated_observable_data([(p, 0, 0.9), (p, -1, 0.8)])
    bases, _ = exponential_fit_curves(averaged)
    assert list(bases) == [p]
    assert bases[p] == pytest.approx(0.8)


def test_exponential_fit_curves_restricts_to_paths(make_cz_path, make_aggregated_observable_data):
    p0, p1 = make_cz_path("XI"), make_cz_path("IX")
    averaged = make_aggregated_observable_data([(p0, -1, 0.8), (p1, -1, 0.7)])
...
def test_averaged_data_points_extracts_point_rows_with_stds(
    make_cz_path, make_aggregated_observable_data
):
    p = make_cz_path("XI")
    averaged = make_aggregated_observable_data([(p, -1, 0.8), (p, 0, 0.9, 0.01), (p, 2, 0.7, 0.02)])
...
def test_dataset_paths_unique_first_seen_order(make_cz_path, make_aggregated_observable_data):
    p0, p1 = make_cz_path("XI"), make_cz_path("IX")
    averaged = make_aggregated_observable_data([(p0, -1, 0.8), (p1, -1, 0.7), (p0, 0, 0.9)])
