...
def _make_observable_data(
    ...
    seed: int = 42,
) -> tuple[ObservableData, dict[str, MockPathPattern]]:
...
class TestAnalysisPipelineIntegration:
    ...
    def test_curve_fit_then_nnls_single_pattern(self):
        """Test curve fitting then linear solving."""
        a_true, f_true = 0.9, 0.8
        pp_name = "pp0"
        obs, patterns = _make_observable_data(
            pattern_depths={pp_name: [1, 2, 3, 4, 5]},
            a_values={pp_name: a_true},
            f_values={pp_name: f_true},
        )
        model = MockFidelityModel({patterns[pp_name]: IndexedVector({"theta": 1.0})})
    ...
    def test_curve_fit_then_nnls_multiple_patterns(self):
        """Test curve fitting then linear solving with multiple patterns."""
        pp0, pp1 = "pp0", "pp1"
        f0, f1 = 0.9, 0.7
        obs, patterns = _make_observable_data(
            pattern_depths={pp0: [1, 2, 3, 4, 5], pp1: [1, 2, 3, 4, 5]},
            a_values={pp0: 0.95, pp1: 0.85},
            f_values={pp0: f0, pp1: f1},
        )
        model = MockFidelityModel(
            {
                patterns[pp0]: IndexedVector({"r0": 1.0}),
                patterns[pp1]: IndexedVector({"r1": 1.0}),
            }
        )
