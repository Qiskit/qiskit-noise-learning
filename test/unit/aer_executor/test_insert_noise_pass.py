...
def test_noise_after_true_injects_at_r_barriers():
    ...
    result = PassManager(
        [InsertNoisePass(noise_dict=_NOISE_DICT, noise_after=True)]
    ).run(qc)
...
def test_noise_after_false_injects_at_m_barriers():
    ...
    result = PassManager(
        [InsertNoisePass(noise_dict=_NOISE_DICT, noise_after=False)]
    ).run(qc)
    assert len(_noise_error_ops(result)) == 1
...
def test_noise_scale_multiplies_rates():
    ...
    result_1x = PassManager(
        [InsertNoisePass(noise_dict=noise_dict, noise_scale=1.0)]
    ).run(qc)
    result_3x = PassManager(
        [InsertNoisePass(noise_dict=noise_dict, noise_scale=3.0)]
    ).run(qc)

    # PauliLindbladError is stored as ._quantum_error inside QuantumChannelInstruction.
    np.testing.assert_allclose(
        _noise_error_ops(result_1x)[0]._quantum_error.rates, [0.1]
    )  # noqa: SLF001
    np.testing.assert_allclose(
        _noise_error_ops(result_3x)[0]._quantum_error.rates, [0.3]
    )  # noqa: SLF001
...
def test_missing_tag_leaves_barrier_intact():
    ...
    result = PassManager(
        [InsertNoisePass(noise_dict=noise_dict, warn_absent=False)]
    ).run(qc)
