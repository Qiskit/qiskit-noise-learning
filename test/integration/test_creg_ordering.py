...
def test_observable_of_crossed_measurement():
    ...
    _, registers = ExecutorCircuitGenerator(gate_set).generate_samplex_item(
        [seq], num_randomizations=1
    )
    (register,) = registers
    assert register.name == "meas0"
    assert register.qubit_idxs == (1, 0)

    # a single shot outcome of 1 on physical qubit 0 and 0 on physical qubit 1
    data = np.zeros((1, 1, 2), dtype=bool)
    data[..., register.qubit_idxs.index(0)] = True
